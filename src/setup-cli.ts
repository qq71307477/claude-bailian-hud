...
import type { InstallState, JsonObject, SettingsFile, StatusLineSnapshot } from './types.js';

interface LegacyOriginalStatusLineFile {
...
function parseCredentials(input: string): { username: string; password: string } {
  const lines = input
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
...
function readLegacyStatusLineSnapshot(): StatusLineSnapshot | null {
  ...
  for (const filePath of legacyFiles) {
    const legacy = readJsonFile<LegacyOriginalStatusLineFile>(filePath);
...
function snapshotCurrentStatusLine(settings: SettingsFile): StatusLineSnapshot {
...
function buildStatuslineScript(runtimePath: string, originalCommand: string | undefined): string {
...
async function main() {
  ...
  const existingState = readJsonFile<InstallState>(paths.installStateFile);
  ...
  writeTextFileAtomic(
    paths.statuslineScriptFile,
    buildStatuslineScript(process.execPath, originalCommand),
    0o755,
  );
