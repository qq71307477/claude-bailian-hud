...
`setup` 时会先尝试一次无头抓取，后续每次新会话开始时也会再无头同步一次，但不会按时间轮询自动刷新。只有你主动运行 `/claude-bailian-hud:fetch` 时，才会切到手动同步模式；这个命令会优先无头，只有无头失败、需要你手动完成一次登录验证时，才打开浏览器兜底。抓取入口会直接打开百炼控制台的 `Coding Plan` 详情页，不再先跳首页。
